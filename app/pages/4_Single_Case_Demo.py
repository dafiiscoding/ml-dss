...
from src.config import CRISIS_KEYWORDS, SAMPLE_DATA_DIR
...
try:
    ...
    with col_in2:
        ...
        img_option = st.selectbox(
            "Choose a method:",
            ["Choose from real CrisisMMD images", "Upload your own image", "Text only"]
        )

        selected_img_path = None
        temp_img_file = None

        if img_option == "Choose from real CrisisMMD images":
            ...
            temp_img_file = st.file_uploader("Upload Image:", type=["png", "jpg", "jpeg"])
            if temp_img_file:
                # Save file to a temporary location
                temp_path = os.path.join(SAMPLE_DATA_DIR, "temp_upload.png")
    ...
    if st.button("🚀 Run DSS Analysis", type="primary"):
        if not tweet_text.strip():
            ...
            with st.spinner("Đang chạy mô hình trích chọn đa phương thức..."):
                # Run prediction
    ...
    st.info("Please make sure you trained the text and image models first.")
